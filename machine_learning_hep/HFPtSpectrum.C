...
//
// Macro taken from AliPhysics, ~Summer 2019
//
// Contains:
//  - HFPtSpectrum():
//       The standard AliPhysics macro
//  - HFPtSpectrum2()
//       Mimic of the calculations performed in the AliHFPtSpectrum class
//       Used for vs-multiplicity studies, where we take fPrompt from MB case

//
// HFPtSpectrum execution parameters:
//  0) Option for decay channel
//  1) filename with the theoretical predictions  (direct & feed-down)
//  2) acceptance and reconstruction efficiencies file name (direct & feed-down)
//  3-4) Names of efficiency histograms
//  5) Raw yield file name
//  6) Name of yield histogram
//  7) output file name
//  8-9) Set the luminosity: the number of events analyzed, and the cross-section of the sample [pb]
//  10) Set the feed-down calculation option flag: knone=none, kfc=fc only, kNb=Nb only
//  11) Option for collision system
//  12) Option for year
//  13) Option for energy
//  14) Int_t isRaavsEP=kPhiIntegrated
//  15)  const char *epResolfile=""
//  16) Flag to decide if there is need to evaluate the dependence on the energy loss
//  17) Int_t rapiditySlice=kdefault
//  18) Set whether the yield is for particle + anti-particles or only one of the 'charges'
//  19) Int_t analysisSpeciality=kTopological
//  20) Int_t ccestimator = kV0M
//  21) Bool_t setUsePtDependentEffUncertainty=true,
//  22) Name of histogram containing number of events (in raw yield file)

//
// HFPtSpectrum2 execution parameters:
//  0) Inputfilename from HFPtSpectrum() containing fPrompt that should be used
//  1) Trigger efficiency
//  2) Trigger efficiency uncertainty
//  3) Acceptance and reconstruction efficiencies file name (direct & feed-down)
//  4-5) Names of efficiency histograms
//  6) Raw yield file name
//  7) Name of yield histogram
//  8) output file name
//  9-10) Set the luminosity: the number of events analyzed, and the cross-section of the sample [pb]
//  11) Set whether the yield is for particle + anti-particles or only one of the 'charges'
//  12) setUsePtDependentEffUncertainty
//  13) Name of histogram containing number of events (in raw yield file)
//
// Macro to mimic AliHFPtSpectrum class using HFPtSpectrum file as input
...
void HFPtSpectrum ( Int_t decayChan=kDplusKpipi,
  ...
  if(PbPbEloss){
    AliHFPtSpectrum *CalcBins = new AliHFPtSpectrum(); //=NULL;
    gStyle->SetPalette(1);
...
void HFPtSpectrumRescaled ( Int_t decayChan=kDplusKpipi,
                   const char *mcfilename="FeedDownCorrectionMC.root",
                   const char *efffilename="Efficiencies.root",
                   const char *nameeffprompt= "eff",
                   const char *nameefffeed = "effB",
  ...
  // Scale FONLL inputs if we do the analysis in y-slices
  //
  hFeedDownMCpt->Scale(2);
  hFeedDownMCptMax->Scale(2);
  hFeedDownMCptMin->Scale(2);
